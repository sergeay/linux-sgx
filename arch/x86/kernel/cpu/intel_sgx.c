...
unsigned long get_total_sgx_mem_kb(int i)
{
	struct sgx_epc_node *node = SGX_NODE_DATA(i);

	return node->nr_total_pages << 2;
}
EXPORT_SYMBOL(get_total_sgx_mem_kb);

unsigned long get_free_sgx_mem_kb(int i)
{
	struct sgx_epc_node *node = SGX_NODE_DATA(i);

	return node->free_cnt << 2;
}
EXPORT_SYMBOL(get_free_sgx_mem_kb);
